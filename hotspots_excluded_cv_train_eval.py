...
class CV_Pipeline:
    ...
    def test_specific_subset(self, histogram_size, exclude_outlier_cores, p, predictive_channel, path_dir):
        ...
        is_printed = False
        reports = dict()
        processing = min_max_exclude_hotspots(percentiles, histogram_size, exclude_outlier_cores)
        ...
        for i, (train_index, test_index) in enumerate(tqdm(list(kf.split(np.arange(len(core_ids)), y, analysis_ids)))):
            ...
            for model_seed in model_seeds_list:
...
def main():
    experiments_dir = os.path.join('.', 'results', 'hotspots_excluded_cv_train_eval')
